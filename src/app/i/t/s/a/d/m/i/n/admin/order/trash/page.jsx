// // ROUTE: /i/t/s/a/d/m/i/n/admin/orders/trash
// // Trash page — shows every order that has been moved to trash, along with
// // the exact date/time it was trashed. Lets an authorized person restore an
// // order or permanently delete it (with a name-confirmation warning).
...
//   AlertOctagon,
// } from "lucide-react";
// import { dummyOrders, STATUS_MAP, calcOrderTotal } from "../../components/data/orders";
// import { getOrderStatus } from "../../lib/Statusstore";
// import {
//   getTrashMap,
//   restoreFromTrash,
//   permanentlyDelete,
...
//   const [deleteTarget, setDeleteTarget] = useState(null); // order pending permanent delete

//   useEffect(() => {
//     const refresh = () => setTrashMap(getTrashMap());
//     refresh();
//     const unsubscribe = subscribeToTrashChanges(refresh);
//     return unsubscribe;
//   }, []);

//   const trashedOrders = useMemo(() => {
//     return dummyOrders
//       .filter((o) => trashMap[o.id])
//       .map((o) => ({ ...o, trashedAt: trashMap[o.id].trashedAt }))
//       .sort((a, b) => new Date(b.trashedAt) - new Date(a.trashedAt));
//   }, [trashMap]);

//   const handleRestore = (order) => {
//     restoreFromTrash(order.id);
//     setToast(`${order.id} restored from trash`);
//   };

//   const confirmPermanentDelete = (confirmedName) => {
//     if (deleteTarget) {
//       permanentlyDelete(deleteTarget.id);
//       setToast(`${deleteTarget.id} permanently deleted by ${confirmedName}`);
//     }
...
//         title={`Permanently delete ${deleteTarget?.id || ""}`}
...
// FILE PATH: app/i/t/s/a/d/m/i/n/admin/orders/trash/page.jsx
// ROUTE: /i/t/s/a/d/m/i/n/admin/orders/trash
// 🟢 Ab backend se trashed orders fetch karta hai (dummyOrders + trashStore hata diye)
