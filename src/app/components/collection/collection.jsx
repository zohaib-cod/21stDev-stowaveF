...
export default function Example() {
  // Pehli image ko by default expand rakhne ke liye 0 set kiya hai, aap null bhi rakh sakte hain
  ...
  // Baahar click karne par normal karne ka logic
  useEffect(() => {
    function handleClickOutside(event) {
      if (galleryRef.current && !galleryRef.current.contains(event.target)) {
        setActiveIndex(null); // Saari images normal size mein aa jayengi
      }
    }

    document.addEventListener("mousedown", handleClickOutside);
    document.addEventListener("touchstart", handleClickOutside); // Mobile touch ke liye
  ...
  return (
    <>
      ...
      <section className="w-full flex flex-col items-center justify-start py-12 min-h-screen">
        ...
        {/* Gallery Container - ref={galleryRef} baahar ka click check karne ke liye hai */}
        <div 
          ref={galleryRef}
        ...
        >
          {images.map((src, idx) => {
            ...
            return (
              <div
                key={idx}
                // Ab onClick sirf direct index set karega, toggle nahi karega
